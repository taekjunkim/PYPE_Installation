...
SegmentationCues_NPX.py is a Fixation task for V4 recording with texture stimuli. It creates visual stimuli from jpg files.
...
def main(app):
    app.taskObject = SegmentationCues(app)
    app.globals = Holder()
    app.idlefb()
    app.startfn = RunSet
...
class SegmentationCues: 
    ...
    def createStimuli(self,app):
        ...
        #myScale = int(600 * RF_size / 300) # 200 RF size assumed when jpg file was created
        myScale = RF_size; 

        png_folder = '/home/shapelab/.pyperc/Tasks/Taekjun/stimFiles/KianiTop200_RemovedBG/'
        #jpg_files = os.listdir(jpg_folder)
        #jpg_files.sort()   # to load jpg images with the right sequence

        ##for loop to read xxx____.jpg, then make sprites for them.
        for num in range(0,101):
            filename = png_folder + 'rank_' + str(num) + '.png'; 
            ...
            s = Sprite(175, 175, myX + eval(params['FixationShift'])[0], 
        ...
        for j in range(0,params['nBlanks']):
            ...
            idNow = idNow + 1
            self.myid.append(idNow)
            print(idNow) ##this should be 102 now
            
        self.fix_shift = np.array([[0,0],[-5,0],[-5,5],[0,5]])*gParam['mon_ppd']; 
        self.fix_shift = self.fix_shift.astype(int); 

        ## make stimulus matrix 
        # each trial has 3 stim flashes at a fixation location
        stim_mtx = np.empty((0,4),int); 
        for rep in np.arange(10):     ### 10 repetitions
            for fp in np.arange(4):   ### fixation points
                temp = np.arange(1,103); 
                np.random.shuffle(temp); 
                temp = np.reshape(temp,(-1,3)); 
                temp = np.hstack((temp,fp*np.ones((34,1)))); 
                stim_mtx = np.vstack((stim_mtx,temp)); 
        np.random.shuffle(stim_mtx); 
        self.StimInfoTable = stim_mtx.astype(int); 
        self.numTrial = np.shape(stim_mtx)[0]; 
 
        """
        ##need multiple repetition
        ##at every repetition, we shuffled stimulus sequence
        numUniqueStims = len(self.mySprites) ##this should be 102 now
    ...
    def createParamTable(self,app):
        ...
        self.myTaskParams = ParamTable(self.myTaskNotebook, (        
            ...
            ("TaskName","SegmentationCues", is_any, "TaskName"),
...
#                self.mySprites[i].__del__()
    ...
    def encodeISI(self,app,trial_start, scount):
        #sIDIndex = self.mySpriteList[sIndex]
        #import pdb; pdb.set_trace(); 
        fix_id = self.StimInfoTable[trial_start,3]; 
        stim_id = self.StimInfoTable[trial_start, scount]; 
        uniq_stim = np.max(self.StimInfoTable); 
        
        myid = uniq_stim*fix_id + stim_id; 
    ...
    def runSet(self,app):
        ...
        try:
           ...
           while app.running and (app.globals.ncorrect < self.numTrial):			   
    ...
    def _RunTrial(self,app, t):
        ...
        ### fixaiton position varied across trials
        trial_start = app.globals.ncorrect; 
        fix_id = self.StimInfoTable[trial_start,3]; 

        # Fixation position at P['fix_x'] and P['fix_y'], 
        # which are in the monk_params table
        fx, fy = P['fix_x'], P['fix_y']
        fx = fx + eval(params['FixationShift'])[0] + self.fix_shift[fix_id,0]; 
        fy = fy + eval(params['FixationShift'])[1] + self.fix_shift[fix_id,1]; 
        fx = int(fx)       
        fy = int(fy)
        ...
        #import pdb; pdb.set_trace(); 			        
        sshow = np.shape(self.StimInfoTable)[1]-1; 
        Numframes = self.Numframes
        for i in arange(0,sshow):
            spr_num = self.StimInfoTable[trial_start,i];  
            spr = self.getSprite(spr_num); 
            spr.x = params['RF_Center_X'] + eval(params['FixationShift'])[0] + self.fix_shift[fix_id,0]; 
            spr.y = params['RF_Center_Y'] + eval(params['FixationShift'])[1] + self.fix_shift[fix_id,1];  
            stim_arr.append(spr)
            stim_arr[i].off()
            app.globals.dlist.update()
        #import pdb; pdb.set_trace(); 			
		...
		# I want to know how long it's been since then.)
            ...
            while scount < sshow:
                P = self.myTaskParams.check(mergewith=app.getcommon())
                self.encodeISI(app, trial_start, scount)
    ...
    def saveEvents(self,app):# Tomo Neuropixles Mar 8 2019 # edited for Neuropixel 08/08/2020 Taekjun
        ...
        if self.taskparam==0:
            ...
            Header = ['RF_x',int(params['RF_Center_X']),'RF_y',int(params['RF_Center_Y']),
                      'num_cond',121,'V2orV4',int(params['V2orV4']),
