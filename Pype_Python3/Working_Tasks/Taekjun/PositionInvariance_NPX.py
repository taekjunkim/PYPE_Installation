...
def main(app):
    app.taskObject = PositionInvariance(app)
    app.globals = Holder()
    app.idlefb()
    app.startfn = RunSet
...
class PositionInvariance:
    ...
    def createStimuli(self,app):
        ...
        ### Choose 100 Shapes (Random 99 Shapes + 1 big circle)
        stim_rot = np.array(stim_rot); 
        shape_chosen = np.concatenate(([1],np.random.choice(np.arange(3,366),99))); 
        shape_chosen.sort(); 
        ...
        idNow = 0; 
        df = pd.DataFrame(columns=['cond_num','shape_id','pos_id']); 
        pos_shift = np.array([[0,0],[1,0],[0,-1],[-1,0],[0,1]])*RF_dia*0.25; 
        for shape_stim in range(100):
            shape_id = shape_chosen[shape_stim]; 
            stim_id = stim_rot[shape_id,1]+1;    # stim_id begins from 1 in b8StimFactory
            rotation = 45*stim_rot[shape_id,2]; 

            s = myFactory.getB8Stim(
                                    stim_id,self.sampling,myFB,(100,100,100),rotation,
                                    RF_X + eval(params['FixationShift'])[0],
                                    RF_Y + eval(params['FixationShift'])[1],
                                    (40,40,40),sp_h_offset=0,sp_v_offset=0, sp_scaling=1,depth=1)   
            for pos_id in range(5):
                sNow = s.clone();                 
                sNow.x = RF_X + eval(params['FixationShift'])[0] + pos_shift[pos_id][0]; 
                sNow.y = RF_Y + eval(params['FixationShift'])[1] + pos_shift[pos_id][1]; 
    ...
    def createParamTable(self,app):
        ...
        self.myTaskParams = ParamTable(self.myTaskNotebook, (        
            ...
            ("SampleSizeFract", "0.5", is_float, "Fraction of RF that the size of the stimulus is suppose to be."),
            ...
            ("TaskName","PositionInvariance", is_any, "TaskName"),
    ...
    def saveEvents(self,app):# Tomo Neuropixles Mar 8 2019 # edited for Neuropixel 02/09/2022 Taekjun
        ...
        if self.taskparam==0:
            ...
            Header = ['RF_x',int(params['RF_centerX']),'RF_y',int(params['RF_centerY']),
                      'num_cond',501,'V2orV4',int(params['V2orV4']),
