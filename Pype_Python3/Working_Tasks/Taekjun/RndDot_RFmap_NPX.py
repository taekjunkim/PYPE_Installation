# -*- coding: utf-8 -*-
"""
Created on Tue Mar 1 18:40:54 2016

RndDot_RFmap.py is a Fixation task for V4 RF mapping with random dot stimuli
...
def main(app):
    app.taskObject = RndDot_RFmap(app)
    app.globals = Holder()
    app.idlefb()
    app.startfn = RunSet
...
class RndDot_RFmap:
    ...
    def createStimuli(self,app):
        ...
        MatFileBase = '/home/shapelab/.pyperc/Tasks/Taekjun/stimFiles/RFmap'
        #MatFileName = os.path.join('Taekjun', 'TextureStim.mat')
        MatFileName = os.path.join(MatFileBase, 'DotPoints.mat')
        
        sData = spio.loadmat(MatFileName)
        #import pdb; pdb.set_trace()

        idNow = 0
        myScale = shape(sData['DotPoints'][0][0]['StimMtx'])
        myScale = myScale[0] ### 54: about 1.5deg (37pixels/deg)
        
        ## Make random dot stimuli
        for i in arange(0,r_Xmax-r_Xmin+1):
            myX = (r_Xmin + i)*int(gParam['mon_ppd'])
            for j in arange(0,r_Ymax-r_Ymin+1):
                myY = (r_Ymin + j)*int(gParam['mon_ppd'])
                s = [None]*self.Numframes
                for k in arange(0,self.Numframes):
                    s[k] = Sprite(myScale, myScale, myX + eval(params['FixationShift'])[0], 
                                  myY + eval(params['FixationShift'])[1], fb=myFB, depth=1, on=0,centerorigin=1)   
                    s[k].alpha_gradient(myScale/2-3, myScale/2, x=0, y=0)		                 

                    imageNow = np.zeros((myScale,myScale,3))
                    imageNow[0:myScale,0:myScale,0] = sData['DotPoints'][0][k]['StimMtx']
                    imageNow[0:myScale,0:myScale,1] = sData['DotPoints'][0][k]['StimMtx']
                    imageNow[0:myScale,0:myScale,2] = sData['DotPoints'][0][k]['StimMtx']
    ...
    def createParamTable(self,app):
        ...
        self.myTaskParams = ParamTable(self.myTaskNotebook, (        
            ...
            ("nstim", "10", is_int, "Number of stimuli"),
            ...
            ("TaskName","RndDot_RFmap_g1_t1", is_any, "TaskName"),
