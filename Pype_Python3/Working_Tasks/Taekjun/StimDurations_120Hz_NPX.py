...
def main(app):
    app.taskObject = StimDurations(app)
    app.globals = Holder()
    app.idlefb()
    app.startfn = RunSet
...
class StimDurations:
    ...
    def createStimuli(self,app):
        ...
        ### Make shapes to be used - 1 Shape, 8 Rotations
        ShapeIDs = eval(params['ShapeIDs'])
        ...
        for j in range(0,8):
            ...
            s = myFactory.getB8Stim(
                                    ...
                                    myBG,sp_h_offset=0,sp_v_offset=0, sp_scaling=1,depth=1)   
            self.mySprites.append(s);             
        # mySprites[0]~mySprites[7]: 8 rotations of a shape

        BGlist = None        
        for j in arange(0,params['nBlanks']):
            s = createBar(30, 30, myFB,myBG, 0, RF_X, RF_Y, myBG)
            self.mySprites.append(s)
        # mySprites[8]: blank stimulus

        ### stimulus information matrix
        # 80 trials: 400 ms duration
        stimNumbers = np.arange(1,10); 
        t400 = np.empty((80,9),dtype=int); 
        for r in np.arange(80):
            t400[r,:] = np.random.permutation(stimNumbers); 
        # 40 trials: 200 ms duration        
        stimNumbers = np.arange(1,10) + 9; 
        t200 = np.empty((40,9*2),dtype=int); 
        for r in np.arange(40):
            repeated_sequence = np.array([], dtype=int)
            for j in range(2):
                repeated_sequence = np.concatenate((repeated_sequence, np.random.permutation(stimNumbers)))
            t200[r,:] = repeated_sequence; 
        # 20 trials: 100 ms duration    
        stimNumbers = np.arange(1,10) + 18;             
        t100 = np.empty((20,9*4),dtype=int); 
        for r in np.arange(20):
            repeated_sequence = np.array([], dtype=int)
            for j in range(4):
                repeated_sequence = np.concatenate((repeated_sequence, np.random.permutation(stimNumbers)))
            t100[r,:] = repeated_sequence; 
        # 10 trials: 50 ms duration                  
        stimNumbers = np.arange(1,10) + 27;             
        t50 = np.empty((10,9*8),dtype=int); 
        for r in np.arange(10):
            repeated_sequence = np.array([], dtype=int)
            for j in range(8):
                repeated_sequence = np.concatenate((repeated_sequence, np.random.permutation(stimNumbers)))
            t50[r,:] = repeated_sequence; 
        # 5 trials: 25 ms duration                   
        stimNumbers = np.arange(1,10) + 36;                                  
        t25 = np.empty((5,9*16),dtype=int); 
        for r in np.arange(5):
            repeated_sequence = np.array([], dtype=int)
            for j in range(16):
                repeated_sequence = np.concatenate((repeated_sequence, np.random.permutation(stimNumbers)))
            t25[r,:] = repeated_sequence; 

        df = pd.concat([pd.DataFrame(t400), pd.DataFrame(t200), pd.DataFrame(t100), 
                        pd.DataFrame(t50), pd.DataFrame(t25)], axis=0)
        df.reset_index(drop=True, inplace=True);      
        df = df.astype('Int64'); 
        self.stim_table = df; 
        
        numTrials = np.shape(df)[0]
        print('numTrials: '+str(int(numTrials))); 
        trialNumbers = np.arange(0,numTrials);
        if (randomize_stims):
            np.random.shuffle(trialNumbers);

        self.myTrialList = trialNumbers; 
        self.numTrial = numTrials
    ...
    def createParamTable(self,app):
        ...
        self.myTaskParams = ParamTable(self.myTaskNotebook, (        
            ...
            ("nBlanks", "1", is_int, "Number of blank trials"),            
            ...
            ("ShapeIDs", "[16]", is_any, "choose 1 Shape ID"),
            ...
            ("IStime", "0", is_int, "Fixed Inter-stimulus interval"),
            ("stimon", "400", is_int, "Fixed Stimulus presentation"),
            ("nstim", "9", is_int, "Number of stimuli"),
            ...
            ("TaskName","StimDurations_120Hz", is_any, "TaskName"),
...
#                self.mySprites[i].__del__()
    ...
    def encodeISI(self,app,tNum, scount):

        myid = self.stim_table.iloc[tNum, scount];         
    ...
    def _RunTrial(self,app, t):
        ...
        tNum = self.myTrialList[trial_start]; 
        ...
        Numframes = int(120*stim_dur/1000); 
        stim_seq = self.stim_table.iloc[tNum,:sshow]; 

        print("stim_dur: "+str(stim_dur)+" ms"); 
        stim_arr = []; #empty list
        for i in arange(0,sshow):
            spr = self.mySprites[stim_seq[i]-offset]; 
            stim_arr.append(spr)
            stim_arr[i].off(); 
        app.globals.dlist.update()
		...
		# I want to know how long it's been since then.)
            ...
            while scount < sshow:
                P = self.myTaskParams.check(mergewith=app.getcommon())
                self.encodeISI(app, tNum, scount)                
                while t.ms() < isi:
